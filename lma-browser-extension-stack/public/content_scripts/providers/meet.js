...
const checkForMeetingMetadata = function () {
	setTimeout(() => {
		try {
			metadata.userName = document
				.querySelector('script[class="ds:8"]')
				.innerText.match(/data:\[(.+?)\]/)[1]
				.split(',')[6]
				.replaceAll('"', '');
...
window.onload = function () {
			...
			if (mutation.type === 'attributes') {
				...
				if (targetElement.getAttribute('jscontroller') == 'ES310d' && mutation.attributeName === 'class') {
					if (targetElement.offsetParent !== null) {
						// Check if the indicator element is visible
						const container = targetElement.closest('div[data-participant-id]');
						const activeSpeaker = container.querySelector(
							'[data-tooltip-id][data-tooltip-anchor-boundary-type] span.notranslate'
						).innerText;
	...
	const activeSpeakerInterval = setInterval(() => {
		...
		if (speakerPanel) {
			...
			activeSpeakerObserver.observe(speakerPanel, {
				subtree: true,
				childList: true,
				attributes: true,
